...
const YEBOSmartSearchResults = ({ data, compact = false, premium = false }) => {
  if (!data) return null;
  ...
  return (
    <div className={`space-y-3 yebone-fade-up ${compact ? "" : "mt-4"}`}>
      ...
      {!premium && data.searchRequest && (
        <div className="flex flex-wrap gap-1.5">
          {data.searchRequest.brand && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800">
              Brand: {data.searchRequest.brand}
            </span>
          )}
          {data.searchRequest.category && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800">
              Category: {data.searchRequest.category}
            </span>
          )}
          {data.searchRequest.maxPrice != null && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800">
              Max: RWF {formatPrice(data.searchRequest.maxPrice)}
            </span>
          )}
          {data.searchRequest.minPrice != null && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800">
              Min: RWF {formatPrice(data.searchRequest.minPrice)}
            </span>
          )}
        </div>
      )}
