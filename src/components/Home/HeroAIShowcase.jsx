...
const GlassPanel = ({ children, className = "" }) => (
  <div
    className={`home-glass rounded-2xl border border-white/10 bg-white/[0.06] shadow-xl backdrop-blur-xl ${className}`}
  >
    {children}
  </div>
);
...
const HeroAIShowcase = () => {
  ...
  return (
    <div className="relative w-full min-h-[380px] sm:min-h-[440px] lg:min-h-[540px] home-fade-up">
      {/* Ambient depth */}
      <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-br from-[#0d1f1c] via-[#0a1211] to-[#1a1510] border border-white/[0.08] shadow-2xl shadow-yebone-primary/20 overflow-hidden">
        <div className="absolute top-0 right-0 w-72 h-72 bg-yebone-primary/20 rounded-full blur-[80px]" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-yebone-gold/10 rounded-full blur-[70px]" />

        {/* App chrome */}
        <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-4 sm:px-5 py-3 border-b border-white/[0.06] bg-black/20">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 home-pulse-dot" />
            <span className="text-[11px] font-semibold text-white/90 tracking-wide">
              Yebone Virtual Try-On
            </span>
          </div>
          <span className="text-[10px] text-yebone-gold/90 font-medium px-2 py-0.5 rounded-full bg-yebone-gold/10 border border-yebone-gold/20">
            Africa · AI-first
          </span>
        </div>
        ...
        <div className="relative z-[5] flex flex-col items-center justify-center h-full min-h-[380px] sm:min-h-[440px] lg:min-h-[540px] pt-12 pb-6 px-4 sm:px-6">
          ...
          <div className="relative w-[140px] sm:w-[170px] lg:w-[190px] aspect-[3/4] mb-4">
            ...
            >
              ...
              <path
                d="M35 58 Q60 48 85 58 L92 120 Q60 132 28 120 Z"
                fill="url(#figureGrad)"
                opacity="0.9"
              />
              <path
                d="M28 120 Q20 145 35 155 L85 155 Q100 145 92 120"
                fill="url(#figureGrad)"
                opacity="0.75"
              />
            </svg>
          ...
          <div className="flex gap-2 sm:gap-3 w-full max-w-md">
            <GlassPanel className="flex-1 p-2 sm:p-3">
              <p className="text-[9px] uppercase tracking-wider text-white/50 mb-1.5">Before</p>
            ...
            <GlassPanel className="flex-1 p-2 sm:p-3 ring-1 ring-yebone-gold/30">
              <p className="text-[9px] uppercase tracking-wider text-yebone-gold mb-1.5 flex items-center gap-1">
                <HiOutlineSparkles size={10} /> After · AI
              </p>
              <div className="aspect-square rounded-xl overflow-hidden bg-black/30 relative">
        ...
        {suggestions.map((product, i) => (
          ...
          >
            <p className="text-[8px] font-bold text-yebone-gold uppercase mb-1">
            ...
            <p className="text-[9px] text-white/80 line-clamp-1">{product?.name}</p>
          </GlassPanel>
        ))}

        {/* Metrics bar */}
        <div className="absolute bottom-4 sm:bottom-5 inset-x-4 sm:inset-x-5 z-20">
          <GlassPanel className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3">
            <div>
              <p className="text-[9px] text-white/50 uppercase tracking-wider">Confidence</p>
              <p className="text-lg sm:text-xl font-bold text-yebone-gold">94%</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div className="text-center">
              <p className="text-[9px] text-white/50 uppercase tracking-wider">AI Match</p>
              <p className="text-sm font-semibold text-emerald-400">Excellent</p>
            </div>
            <div className="h-8 w-px bg-white/10 hidden sm:block" />
            <div className="hidden sm:block text-right">
              <p className="text-[9px] text-white/50 uppercase tracking-wider">Platform</p>
              <p className="text-sm font-semibold text-white">Yebone AI</p>
            </div>
          </GlassPanel>
        </div>
      </div>
